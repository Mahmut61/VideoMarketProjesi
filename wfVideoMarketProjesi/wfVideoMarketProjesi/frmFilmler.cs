...
namespace wfVideoMarketProjesi
{
    public partial class frmFilmler : Form
    {
        ...
        {
            if (txtFilAdi.Text.Trim() != "" && txtTurNo.Text.Trim() != "" && txtYonetmen.Text.Trim() != "")
            {
                cFilm f = new cFilm();
                bool Sonuc = f.FilmVarmi(txtFilAdi.Text, txtYonetmen.Text);
                if (Sonuc)
                {
            ...
            else { MessageBox.Show("Film Adı,Türü ve Yönetmen alanları boş geçilemez!", "Dikkat Eksik Bilgi"); }
        }

        private void lvFilmler_DoubleClick(object sender, EventArgs e)
        {
            txtFilmNo.Text = lvFilmler.SelectedItems[0].SubItems[0].Text;
            txtFilAdi.Text = lvFilmler.SelectedItems[0].SubItems[1].Text;
        ...
        private void btnDegistir_Click(object sender, EventArgs e)
        {
            ...
            {
                cFilm f = new cFilm();
                bool Sonuc = f.FilmVarmi(txtFilAdi.Text, txtYonetmen.Text,txtFilmNo.Text);
                if (Sonuc)
                {
                    MessageBox.Show("Bu film önceden kayıtlı!");
                    txtFilAdi.Focus();
                ...
                else
                {
                    ...
                    {
                        MessageBox.Show("Film Değiştirmede sorunla karşılaşıldı!");
                        txtFilAdi.Focus();
                    }
                }
            }
        ...
        private void btnSil_Click(object sender, EventArgs e)//ben yaptım
        {
